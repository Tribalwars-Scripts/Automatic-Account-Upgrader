// ==UserScript==
// @name                Automatic Village Upgrade TribalWars
...
// @description 	    Script to automatically upgrade the village.
// @codigo              Script made in JavaScript forked from https://github.com/tribalwarsbr100/Upador-Tribal-Wars/
// @author		        Bruno Preto
// @include             **tribalwars*game.php*screen=main*
// @version     	    0.1
...
const Min_Tempo_Espera= 900000;
const Max_Tempo_Espera = 1000000;
...
setInterval(function(){
    ...
    //Check if the Missions popup is active
    let popupbox = document.getElementsByClassName("popup_box slim show");
    if (popupbox.length){
        //Check if there's any rewards to be collected
        let RewardToCollect = document.querySelector("#reward-system-badge");
        let flag = false;
        if (RewardToCollect.innerHTML.trim().substring(1,2) > '0' && document.getElementsByClassName("small warn").length === 0){
            if (!flag){
                RewardToCollect.click();
                flag=true;
            }
            //There are rewards to be collected and the warn is not full
            document.querySelector("#reward-system-rewards > tr:nth-child(1) > td:nth-child(6) > a").click();
        }
        //Switch to Normal Missions view
        document.getElementsByClassName("tab-link")[0].click();
        let questsvar = document.getElementsByClassName("quest-link");
        for (let j=0;j<questsvar.length;j++){
            questsvar[j].click();
            if (document.getElementsByClassName("skip-btn btn").length > 0 ){
                // Still buggy will request a lot of shit
                //document.getElementsByClassName("skip-btn btn")[0].click();
            }
            if (document.getElementsByClassName("quest-complete-btn").length){
                // Possible to finish Mission
                document.getElementsByClassName("quest-complete-btn")[0].click();
            }
        }
    }
...
//
//     //This can be after the storage building.
//
//
...
//
//     // Done, the remaining is up to the users choice
//
